			...
			<Grid templateColumns={'repeat(12, 1fr)'} gap={4}>
				<GridItem colSpan={12}>
					<Flex gap={4} marginBottom={'8'}>
						...
						<Button isDisabled={!voice} background={'green.400'} isLoading={loading} onClick={handleChangeTextToSpeech}>
							Tạo File Giọng Nói
						</Button>
						<Button
						...
						<Button colorScheme='messenger' onClick={() => setModal({ ...modal, open: true })}>
							{`Xem danh sách audio (${checkFormEmpty(form).length} bản ghi)`}
						</Button>
					</Flex>
					{form.length > 0 &&
						form.map((_i, idx) => {
							return (
								<Flex key={idx} marginBottom={'2'} gap={2}>
									<Text>{_i.id}.</Text>
